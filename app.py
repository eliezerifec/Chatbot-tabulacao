...
class App:
    ...
    def _sidebar(self):
        ...
        nav = [("o", False), ("=", True), ("*", False), ("+", False), ("~", False)]
        for icon, active in nav:
            bg = NAV_ACTIVE if active else NAV_BG
            f  = tk.Frame(sb, bg=bg, height=52, cursor="hand2")
            f.pack(fill=tk.X)
            f.pack_propagate(False)
            if active:
                tk.Frame(f, bg="#60a5fa", width=3).pack(side=tk.LEFT, fill=tk.Y)
            tk.Label(f, text=icon, bg=bg,
                     fg="#f9fafb" if active else "#6b7280",
                     font=F_NAV).pack(expand=True)
            if not active:
    ...
    def _main(self):
        ...
        main.grid_rowconfigure(1, weight=1)  # scroll
        ...
        # Frame interno do canvas — expande com a janela
        self.sf = tk.Frame(canvas, bg=BG)
        self._sf_id = canvas.create_window((0, 0), window=self.sf, anchor="nw")

        # Responsividade: sf sempre tem a largura do canvas
    ...
    def _card_upload(self, parent):
        ...
        self.lbl_arquivo.pack(anchor="w", pady=(8, 0))
    ...
    def _run_all(self, selecionadas, ctx_global):
        ...
        for idx_aba, (nome, cfg) in enumerate(selecionadas):
            ...
            try:
                # Categorias pré-definidas: usa as da aba (semiaberta)
                # se não tiver, cai para as categorias globais
                cats_imp = cfg.get("cats_aba") or self.codificador.categorias[:]
